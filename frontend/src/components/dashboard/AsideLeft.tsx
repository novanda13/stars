...
import { roleAuth } from '@/helpers/cookies';
    ...
    return (
        <aside id="aside-left" className={`${asideLeftKomponen ? 'aside-left__active' : ''} bg-white border-r transition-all duration-300`}>
            <div className="h-full relative flex flex-col font-semibold text-sm">
                ...
                {roleAuth() != "kasir" && (
                    <>
                        <Link href="/produk" className={`${pathname === '/produk' || pathname == '/produk/tambah' ? 'bg-blue-100 border-l-4 border-[#0C085C]' : ''} flex items-center gap-x-1.5 py-4 px-3`}>
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-5">
